...
def alerts_page():
    """Serve alerts.html"""
    alerts_path = os.path.join(PROJECT_ROOT, 'web', 'alerts.html')
    if os.path.exists(alerts_path):
        with open(alerts_path, 'r', encoding='utf-8') as f:
            return f.read(), 200, {'Content-Type': 'text/html'}
    return "alerts.html not found", 404

@app.route('/models')
def models_page():
    """Serve models.html"""
    models_path = os.path.join(PROJECT_ROOT, 'web', 'models.html')
    if os.path.exists(models_path):
        with open(models_path, 'r', encoding='utf-8') as f:
            return f.read(), 200, {'Content-Type': 'text/html'}
    return "models.html not found", 404

@app.route('/investigation')
def investigation_page():
    """Serve investigation.html"""
    investigation_path = os.path.join(PROJECT_ROOT, 'web', 'investigation.html')
    if os.path.exists(investigation_path):
        with open(investigation_path, 'r', encoding='utf-8') as f:
            return f.read(), 200, {'Content-Type': 'text/html'}
    return "investigation.html not found", 404

@app.route('/simulator')
def simulator_page():
    """Serve simulator.html"""
    simulator_path = os.path.join(PROJECT_ROOT, 'web', 'simulator.html')
    if os.path.exists(simulator_path):
        with open(simulator_path, 'r', encoding='utf-8') as f:
            return f.read(), 200, {'Content-Type': 'text/html'}
    return "simulator.html not found", 404
...
if_model = None
if_metadata = None
deep_verifier = None  # Two-Stage Hybrid: LLM Stage 2 model

# UNSW-NB15 Attack Types (9 attack categories + Normal)
ATTACK_TYPES = [
...
def load_models():
    ...
    # Try to load UNSW-NB15 scaler first (new format), fall back to CIC-IDS2017
    unsw_scaler_path = os.path.join(PROJECT_ROOT, "models", "unsw_scaler.pkl")
    if os.path.exists(unsw_scaler_path):
        try:
            scaler = joblib.load(unsw_scaler_path)
            logger.info(f"[OK] Loaded UNSW-NB15 Scaler from {unsw_scaler_path}")
        except Exception as e:
            logger.warning(f"[WARN] Failed to load UNSW scaler: {e}")
            scaler = None
    else:
        # Fall back to CIC-IDS2017 scaler
        try:
            scaler_path = os.path.join(PROJECT_ROOT, "models", "scaler.pkl")
            scaler = joblib.load(scaler_path)
            logger.info(f"[OK] Loaded CIC-IDS2017 Scaler from {scaler_path}")
        except Exception as e:
...
class Alert:
    ...
    stage2_reason: str | None = None  # LLM deep verification reason (Two-Stage Hybrid)
    ...
    def to_dict(self) -> dict:
        ...
        result = {
            ...
            "stage2_reason": self.stage2_reason,  # Two-Stage Hybrid LLM output (can be None)
        }
        return result
...
# UNSW-NB15 Feature Columns (42 numeric features expected by the model)
...
def extract_unsw_features(packet_data: Dict) -> np.ndarray:
    """Extract features from packet data for UNSW-NB15 model input.
    
    Maps input packet data to the 42 UNSW-NB15 numeric features expected by the scaler.
    """
    features = np.zeros(42, dtype=np.float32)
    
    # Map to exactly 42 features based on UNSW_FEATURE_COLUMNS
    for i, col in enumerate(UNSW_FEATURE_COLUMNS):
        val = packet_data.get(col, 0)
        
        # Simple hash-based encoding for categorical columns to match the shape
        if col in ['proto', 'service', 'state'] and isinstance(val, str):
            features[i] = float(hash(val) % 1000) / 100.0
        else:
            try:
    ...
    # Fill any remaining with computed values from basic packet data
    if features[4] > 0 and features[0] > 0:  # spkts and dur
        features[8] = features[4] / features[0]  # compute rate
...
def _get_scaled_features(features: np.ndarray) -> np.ndarray:
    ...
    if scaler is not None:
        ...
        if n_features == 42:
            ...
            # Unknown format - try without column names
            try:
                return scaler.transform(features)
            except Exception:
                return features
...
def predict_hybrid(features: np.ndarray) -> Dict:
        ...
        try:
            ...
            if hasattr(rf_model, "classes_") and len(rf_model.classes_) > 0:
                try:
                    ...
                    # rf_pred is already a string label
                    rf_label = str(rf_pred)
            else:
                rf_label = str(rf_pred)
    ...
    if if_model is not None and not _using_unsw:
        # IF model was trained on 52 features, skip if using 42 features
        try:
...
def process_packet(packet_data: Dict) -> Alert:
    ...
    # Check if we have UNSW-NB15 features
    unsw_indicators = ['dur', 'proto', 'service', 'state', 'spkts', 'dpkts', 'sbytes', 'dbytes']
    has_unsw_features = any(col in packet_data for col in unsw_indicators)
    
    if has_unsw_features:
        # Use UNSW-NB15 feature extraction (42 features)
        features = extract_unsw_features(packet_data)
    else:
        # Extract features from packet data using CIC format (52 features)
...
def _load_real_samples(attack_type: str, n: int = 20) -> List[Dict]:
    """Load real attack samples from the UNSW-NB15 dataset."""
    global _real_samples_cache
    
    cache_key = f"unsw_{attack_type}_{n}"
    ...
    try:
        ...
        # UNSW-NB15 attack category mapping (API type -> dataset category)
        attack_mapping = {
            # Direct mappings
            "fuzzers": "Fuzzers",
            "analysis": "Analysis",
        ...
        data_path = os.path.join(
            PROJECT_ROOT, "data", "unsw_nb15", 
            "Training and Testing Sets", "UNSW_NB15_testing-set.csv"
        )
        
        if not os.path.exists(data_path):
            logger.warning(f"UNSW-NB15 dataset not found at {data_path}")
            return []
        
        # Read only needed columns (numeric features + attack_cat)
        df = pd.read_csv(data_path)
        
        # Filter by attack category
        samples = df[df["attack_cat"] == label].sample(n=min(n, 100), random_state=42)
            ...
            for col in UNSW_FEATURE_COLUMNS:
                if col in row:
                    ...
                    if isinstance(val, str):
                        sample[col] = float(hash(val) % 1000) / 100.0
                    else:
                        sample[col] = float(val) if pd.notna(val) else 0.0
...
def health_check():
    ...
    return jsonify({
        ...
        "models_loaded": {
            ...
            "deep_verifier": deep_verifier is not None,  # Two-Stage Hybrid LLM
            "unsw_nb15": rf_model is not None,  # Dataset indicator
        },
        "timestamp": datetime.now(UTC).isoformat()
    })
...
def simulate():
    ...
    # Handle "all" attack types - UNSW-NB15 attack types
    valid_types = [
        # UNSW-NB15 categories
        "fuzzers", "analysis", "backdoor", "dos", "exploits",
        "generic", "reconnaissance", "shellcode", "worms", "normal",
        # Legacy aliases for backward compatibility
        "ddos", "synflood", "portscan", "slowloris", "bruteforce", "webattacks"
    ]
    if attack_type == "all":
        # Run multiple attack simulations (UNSW-NB15 types)
        all_alerts = []
        attack_types_to_run = ["fuzzers", "dos", "exploits", "reconnaissance"]
        for atype in attack_types_to_run:
            alerts = simulate_attack(atype, target_ip)
            all_alerts.extend(alerts)
        # Add some normal traffic
        normal_alerts = simulate_attack("normal", target_ip)
...
def lookup_threat_intel(ip: str):
    ...
    is_private = octets[0] == 10 or (octets[0] == 172 and 16 <= octets[1] <= 31) or (octets[0] == 192 and octets[1] == 168) or octets[0] == 127
    
    if is_private:
        return jsonify({"ip": ip, "score": 0, "reputation_label": "Internal", "is_malicious": False, "sources": {}, "message": "Private/internal IP"})
    
    # Get geo location
    lat = lon = country = isp = None
    try:
        from src.utils.geoip import GeoIPService
        loc = GeoIPService().lookup(ip)
        if loc:
            lat, lon, country, isp = loc.latitude, loc.longitude, loc.country, loc.isp
    except Exception:
        pass
    
    # Get TI data
    threat_score = is_malicious = 0
    reputation = "Clean"
    sources = {}
    ...
    if ti_client:
        try:
            ...
            reputation = result.reputation.capitalize() if result.reputation else "Clean"
            if result.country:
                country = result.country
            for s in result.sources:
                sl = s.lower()
                if sl == "virustotal":
                    sources["virustotal"] = {"detected": True, "score": threat_score}
                elif sl == "abuseipdb":
                    sources["abuseipdb"] = {"detected": True, "reports": 1 if threat_score > 20 else 0}
                elif sl == "otx":
                    sources["otx"] = {"detected": True, "pulses": 1 if threat_score > 30 else 0}
        except Exception:
            pass
    
    # Generate mock if no TI data
    if not sources:
