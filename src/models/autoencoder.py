"""Autoencoder anomaly detector training for UNSW-NB15."""
...
def _choose_threshold_from_binary_validation(
    validation_errors: np.ndarray,
    y_true: np.ndarray,
) -> float:
    """Choose a threshold using labeled validation data (binary labels: 1=attack)."""
...
def train_autoencoder() -> AutoencoderArtifacts:
    ...
    # We want to train only on BENIGN (Normal) traffic
    benign_mask = split.y_train_multi == BENIGN_LABEL
    x_benign_full = split.x_train[benign_mask]
    ...
    model = build_autoencoder(x_benign_train.shape[1])
    ...
    # Use the pre-computed validation split from unsw_processor for threshold tuning
    # because threshold tuning requires mixed traffic (both normal and attacks)
    validation_errors = _reconstruction_errors(model, split.x_val, batch_size)
    y_val_binary = split.y_val_bin # 0 is normal, 1 is attack
    
    # Adapt threshold choosing function to work with binary labels directly
    threshold = _choose_threshold_from_binary_validation(validation_errors, y_val_binary)
